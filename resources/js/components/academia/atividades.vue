          ...
          <td :colspan="headers.length">
            <div v-if="expanded.length" style="margin-top:15px; margin-bottom:15px">
              <div v-if="expanded[0].observacoes !== descriptionDefaultValue">
                <p class="font-weight-regular"> {{expanded[0].observacoes}} </p>
              </div>
            </div>
          </td>
        </template>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Observações', value: 'data-table-expand', align: 'center', sortable: false },
        { text: 'Participantes | Docentes', value: 'action', align: 'center', sortable: false },
        { text: 'Editar | Remover', value: 'actions', align: 'center', sortable: false },
      ],
      atividades: [],
      editedIndex: -1,
      editedItem: {},
